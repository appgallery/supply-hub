export const MESSAGE = {
  REGISTER_SUCCESS: "User registered successfully.",
  LOGIN_SUCCESS: "Login successful.",
  LOGOUT_SUCCESS: "Logout successful.",

  USER_ALREADY_EXISTS: "User already exists.",
  USER_NOT_FOUND: "User not found.",

  INVALID_CREDENTIALS: "Invalid email or password.",

  ROLE_NOT_FOUND: "Role not found.",

  UNAUTHORIZED: "Unauthorized.",

  TOKEN_EXPIRED: "Token has expired.",

  INVALID_TOKEN: "Invalid token.",

  SOMETHING_WENT_WRONG: "Something went wrong.",
};

export const TOKEN = {
  ACCESS_EXPIRES_IN: "1h",
  REFRESH_EXPIRES_IN: "7d",
};

export enum MediaType {
  IMAGE = "IMAGE",
  VIDEO = "VIDEO",
}

export enum SellerType {
  CLIENT = "CLIENT",
  SUB_CLIENT = "SUB_CLIENT",
}
export enum Role {
  SUPER_ADMIN = "superadmin",
  CLIENT = "client",
  SUB_CLIENT = "subclient",
  ADMIN = "admin"
}

export enum OrderStatus {
  PENDING = "PENDING",
  COMPLETED = "COMPLETED",
  PROCESSING = "PROCESSING",
  APPROVED = "APPROVED",
  REJECTED = "REJECTED",
}

export enum ActivityType {
  CLIENT_CREATED = "CLIENT_CREATED",
  SUB_CLIENT_CREATED = "SUB_CLIENT_CREATED",
  PRODUCT_CREATED = "PRODUCT_CREATED",
  ORDER_CREATED = "ORDER_CREATED",
  ORDER_STATUS_CHANGED = "ORDER_STATUS_CHANGED",
  CLIENT_UPDATED = "CLIENT_UPDATED",
  CLIENT_DELETED = "CLIENT_DELETED",
  SUB_CLIENT_UPDATED = "SUB_CLIENT_UPDATED",
  SUB_CLIENT_DELETED = "SUB_CLIENT_DELETED",
  ORDER_APPROVED = "ORDER_APPROVED",
  ORDER_REJECTED = "ORDER_REJECTED",
  ORDER_COMPLETED = "ORDER_COMPLETED",
  PAYMENT_RECEIVED = "PAYMENT_RECEIVED",
  INVOICE_CREATED = "INVOICE_CREATED",
  ORDER_UPDATED = "ORDER_UPDATED",
  PRODUCT_ACTIVATED = "PRODUCT_ACTIVATED",
  PRODUCT_DEACTIVATED = "PRODUCT_DEACTIVATED"
}

export enum TaxType {
  GST = "GST",
  VAT = "VAT",
  SALES_TAX = "SALES_TAX",
}

export enum AddressType {
  BILLING = "BILLING",
  SHIPPING = "SHIPPING",
}

export enum PaymentStatus {
  PENDING = "PENDING",
  PAID = "PAID",
  FAILED = "FAILED",
  REFUNDED = "REFUNDED",
}

export enum TransactionStatus {
  PENDING = "PENDING",
  SUCCESS = "SUCCESS",
  FAILED = "FAILED",
  REFUNDED = "REFUNDED",
}

export enum InvoiceStatus {
  UNPAID = "UNPAID",
  PAID = "PAID",
  CANCELLED = "CANCELLED",
}

export enum PaymentMethod {
  BANK_REMITTANCE = "BANK_REMITTANCE",
  CHEQUE = "CHEQUE",
  CASH = "CASH",
  OTHER = "OTHER",
  ONLINE = "ONLINE",
}

export enum TallyDeviceStatus {
  ONLINE = "ONLINE",
  OFFLINE = "OFFLINE",
  ERROR = "ERROR",
}

export enum TallySyncType {
    EXPORT_STOCK_GROUP = "EXPORT_STOCK_GROUP",
    EXPORT_STOCK_CATEGORY = "EXPORT_STOCK_CATEGORY",
    EXPORT_STOCK_ITEM = "EXPORT_STOCK_ITEM",
    EXPORT_LEDGER = "EXPORT_LEDGER",
    EXPORT_VOUCHER = "EXPORT_VOUCHER",

    IMPORT_STOCK_GROUP = "IMPORT_STOCK_GROUP",
    IMPORT_STOCK_CATEGORY = "IMPORT_STOCK_CATEGORY",
    IMPORT_STOCK_ITEM = "IMPORT_STOCK_ITEM",
    IMPORT_LEDGER = "IMPORT_LEDGER",
    IMPORT_VOUCHER = "IMPORT_VOUCHER",
}

export enum TallySyncStatus {
  PENDING = "PENDING",
  RUNNING = "RUNNING",
  COMPLETED = "COMPLETED",
  FAILED = "FAILED",
}
